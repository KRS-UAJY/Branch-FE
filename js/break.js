...
for (var i = pieces - 1; i >= 0; i--) {
  $('#popup-kaca').prepend('<div class="piece" style="width:'+pieceW+'px; height:'+pieceH+'px"></div>');
};
...
function breakGlass(from){
  if (from === "reverse"){
    $('.piece').each(function(){
      ...
      TweenLite.to($('#popup-kaca h1'),0.2,{opacity:1, delay: speed});
    });
    return;
  }

  if(!from){
    TweenLite.to($('#popup-kaca h1'),0.2,{opacity:0});
  } else {
    TweenLite.from($('#popup-kaca h1'),0.5,{opacity:0, delay: speed});
  }
